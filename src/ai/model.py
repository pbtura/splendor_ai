...
class Linear_QNet(nn.Module):
    ...
    def save(self, file_name='model.pth'):
        model_folder_path = "C:\\Users\\bucpa\\Documents\\workspace\\SnakeGameAI\\model\\"
...
class QTrainer:
    ...
    def train_step(self,state,action,reward,next_state,done):
        state = torch.tensor(numpy.array(state),dtype=torch.float).cuda()
        next_state = torch.tensor(numpy.array(next_state),dtype=torch.float).cuda()
