...
class GameState(object):
    ...
    def startNewGame(self, randomize: bool = 1):
        ...
        if randomize:
            ...
            self.playersRandomized = True

            # randomize resource deck
            for y in self.resourceDeck.values():
                random.shuffle(y)
            self.resourcesShuffled = True

            # randomize noble deck
            random.shuffle(self.noblesDeck)
            self.noblesShuffled = True
        else:
            self.playersRandomized = False
            self.resourcesShuffled = False
            self.noblesShuffled = False
    ...
    @staticmethod
    def importNobleDeck(reader) -> deque[NobleCard]:
        nobles: deque = deque()
        for row in reader:
    ...
    def withdrawGems(self, player: Player, gems: OrderedDict):
    ...
    def purchaseCard(self, player: Player, deck: int, card: ResourceCard, gems: OrderedDict):
    ...
    @staticmethod
    def canPurchase(cardCost: GemCollection, availableGems: TokenStore, discounts: np.ndarray[int]) -> bool:
    ...
    @staticmethod
    def findAvailableResources(gems: TokenStore, resources: dict[int, list[ResourceCard]],
                               discounts: list[int]) -> list:
        print("filtering")
